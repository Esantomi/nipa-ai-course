...
- 딥러닝 시작하기
  ...
  - [03. 다양한 신경망](#03-다양한-신경망)
    ...
    - [자연어 처리를 위한 데이터 전 처리](#자연어-처리를-위한-데이터-전-처리)
      - [자연어 전 처리 방법](#자연어-전-처리-방법)
    - [영화 리뷰 긍정/부정 분류 RNN 모델 - 데이터 전 처리 실습](#영화-리뷰-긍정부정-분류-RNN-모델---데이터-전-처리)
      - [RNN을 위한 자연어 데이터 전 처리](#RNN을-위한-자연어-데이터-전-처리)
    ...
    - [영화 리뷰 긍정/부정 분류 RNN 모델 - 평가 및 예측 실습](#영화-리뷰-긍정부정-분류-RNN-모델---평가-및-예측)
...
#### 자연어 전 처리 방법
- 원 상태 그대로의 자연어는 전 처리 과정이 필요함
  - 오류 교정(Noise Canceling)
    - 자연어 문장의 스펠링 체크 및 띄어쓰기 오류 교정
    - 예) `"안녕하세요. 반갑스니다." => "안녕하세요. 반갑습니다."`
  - 토큰화(Tokenizing)
    - 문장을 **토큰(Token)** 단위로 나눔
    - 토큰은 어절, 단어 등으로 목적에 따라 다르게 정의
    - 예) `"딥러닝 기초 과목을 수강하고 있습니다." => ['딥', '러닝', '기초', '과목', '을', '수강', '하고', '있습니다', '.']`
  - 불용어 제거(StopWord removal)
    - 불필요한 단어를 의미하는 불용어(StopWord) 제거
    - 한국어 불용어 예) `아, 휴, 아이구, 아이쿠, 아이고, 쉿, 그렇지 않으면, 그러나, 그런데, 하지만, ...`
- **Bag of Words**
  ...
  - 단어들의 순서는 전혀 고려하지 않고, 단어들의 출현 빈도(frequency)에만 집중하는 텍스트 데이터의 수치화 표현 방법
    ...
    - [BoW 자세히 읽기](https://wikidocs.net/22650)
  - 예) `['안녕', '만나서', '반가워'] ['안녕', '나도', '반가워'] => ['안녕':0, '만나서':1, '반가워':2, '나도':3]`
    - 번호는 곧 정수 인덱스
- **Token Sequence**
  - Bag of words에서 단어에 해당되는 인덱스로 변환
  - 모든 문장의 길이를 맞추기 위해 기준보다 짧은 문장에는 패딩을 수행
  - 예) `['안녕', '만나서', '반가워'] ['나도', '만나서', '반가워'] ['만나서', '반가워', '안녕'] ['안녕', '반가워'] => [0, 1, 2] [3, 1, 2] [1, 2, 0] [0, 2, 4]`
    - 마지막 `4`가 곧 padding

### 영화 리뷰 긍정/부정 분류 RNN 모델 - 데이터 전 처리
이번에는 영화 리뷰 데이터를 바탕으로 **감정 분석**을 하는 모델을 학습시켜 보자.
- 영화 리뷰와 같은 자연어 자료는 곧 단어의 연속적인 배열로서, **시계열 자료**라고 볼 수 있다.
- 즉, 시계열 자료(연속된 단어)를 이용해 리뷰에 내포된 감정(긍정, 부정)을 예측하는 분류기를 만들어 보도록 한다.
- 데이터 셋은 **IMDB 영화 리뷰 데이터 셋**을 사용한다.
  - 이 데이터 셋은 훈련용 5,000개와 테스트용 1,000개로 이루어져 있으며, 레이블은 긍정/부정 두 가지로 나뉜다.
- 우선 자연어 데이터를 RNN 모델의 입력으로 사용할 수 있도록 데이터 전 처리를 수행해 보자.

#### RNN을 위한 자연어 데이터 전 처리
RNN의 입력으로 사용하기 위해, 자연어 데이터는 토큰화 및 여러 가지의 데이터 처리가 필요하다.
- 아래와 같이 자연어 데이터가 준비되어 있다면 마지막으로 패딩을 수행하여 데이터의 크기를 통일해야 한다.
  - 첫 번째 데이터 시퀀스  
    ![image](https://user-images.githubusercontent.com/61646760/145673194-b9696d1d-8b82-4a7b-9c7a-7bbe197d54e2.png) 
  - 단어 사전  
...
- 워드 임베딩(Word Embedding)
  ...
  - 워드 임베딩(Word Embedding)은 단어를 벡터로 표현하는 방법으로, 단어를 밀집 표현으로 변환한다.
    - [워드 임베딩 자세히 읽기](https://wikidocs.net/33520)
  - 예) BoW -> Embedding table
    - BoW
      - `['어머니':0, '아버지':1, '친구':2, '회사':3, ...]`
    - Embedding table
      - `0: [1, 3, 0, -2, 0, 0 ]`
      - `1: [2, 2, 0, -1, 0, 0 ]`
      - `2: [0, 0, 1, -1, -2, 0 ]`
      - `3: [0, -1, 0, 0, -1, 3]`
- 기존 다층 퍼셉트론 신경망의 자연어 분류 방식 ([보기](https://user-images.githubusercontent.com/61646760/145675987-871235a3-8950-4d7c-a496-5bd70fdef1af.png))
  - 자연어 문장을 기존 MLP 모델에 적용시키기에는 한계가 있다.
  - 토큰 간의 순서와 관계를 적용할 수 있는 모델은 없을까?
...
- 자연어 분류를 위한 순환 신경망(Recurrent Neural Network)
  - **순환 신경망(Recurrent Neural Network, RNN)**
    ...
    - 출력 값을 두 갈래로 나뉘어 신경망에게 '**기억**'하는 기능을 부여  
      ![image](https://user-images.githubusercontent.com/61646760/145676229-6e765b22-5314-40df-a5a8-369d4970c696.png) 
      - 즉 상태 계산 시, 이전 상태를 사용하게 되는 인공 신경망
      - 은닉층의 노드에서 활성화 함수를 통해 나온 결과 값을 출력층 방향으로 보내면서, 다시 은닉층 노드의 다음 계산의 입력으로도 보낸다.
        - [RNN 자세히 읽기](https://wikidocs.net/22886)
      - `예)` [RNN 기반 자연어 분류](https://user-images.githubusercontent.com/61646760/145676211-7c77df78-8a16-4c42-87cd-fa54ae288e85.png)
    - RNN 기반 자연어 처리 기술
      - Image captioning
      - Chatbot
...
1. 모델을 구현합니다.
   ...
   Dense 레이어는 0, 1 분류이기에 노드를 1개로 하고 activation을 'sigmoid'로 설정합니다.
"""
model = tf.keras.models.Sequential([
    tf.keras.layers.Embedding(1000, embedding_vector_length, input_length = max_review_length),
    tf.keras.layers.SimpleRNN(5),  # 노드 수 5개
...
Train on 5000 samples
Epoch 1/3
5000/5000 - 10s - loss: 0.6936 - accuracy: 0.5048
Epoch 2/3
...
앞에 이어서 이번에는 RNN 모델을 평가하고 예측해 보자.
- Keras에서 RNN 모델의 평가 및 예측을 위해 필요한 함수
  - `model.evaluate(X, Y)`
    - 평가 방법
    - `evaluate()` 메서드는 학습된 모델을 바탕으로 입력한 feature 데이터 `X`와 label `Y`의 loss 값과 metrics 값을 출력한다.
  - `model.predict(X)`
